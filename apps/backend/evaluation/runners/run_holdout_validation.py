...
HOLDOUT_CASE_IDS = ["TC-002", "TC-005", "TC-007", "TC-009", "TC-012", "TC-015", "TC-017", "TC-022", "TC-026", "TC-030"]
GOLDEN_PATH = os.path.join(BACKEND, "evaluation", "datasets", "golden_dataset.jsonl")

async def run_holdout():
    print("=== STARTING TEN-CASE BLIND HOLDOUT VALIDATION ===")
    ...
    try:
        ...
        print(f"[HOLDOUT] Authenticated as user: {user_id}")
    except Exception as e:
        print(f"[HOLDOUT] Auth failed: {e}")
        return
    ...
    with open(GOLDEN_PATH, "r", encoding="utf-8") as f:
        for line in f:
            if line.strip():
                c = json.loads(line)
                if c["test_case_id"] in HOLDOUT_CASE_IDS:
                    cases.append(c)
                    
    # Sort to match order of HOLDOUT_CASE_IDS
    cases.sort(key=lambda x: HOLDOUT_CASE_IDS.index(x["test_case_id"]))
    ...
    if all_ok:
        print("SUCCESS: All 10 holdout cases passed validation!")
    else:
        print("WARNING: Some holdout cases failed.")

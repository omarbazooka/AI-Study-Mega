...
async def test_unsupported_claim_fails_verifier_and_triggers_fallback(
    ...
    mock_retrieve.return_value = RetrievalResult(
        ...
        confidence=0.9,
        chunks=[RetrievedChunk(chunk_id="chunk-abc", document_id="doc-photo-101", user_id="u1", text="photosynthesis text", score=0.9)],
...
async def test_large_document_quiz_uses_map_reduce_routed_by_token_budget(
    ...
    response = client.post("/api/v1/documents/doc-photo-101/summary", json=payload)
    assert response.status_code == 200
    data = response.json()
...
async def test_citations_only_reference_retrieved_chunks(
    ...
    mock_retrieve.return_value = RetrievalResult(
        ...
        confidence=0.9,
        chunks=[
            RetrievedChunk(chunk_id="chunk-photo-1", document_id="doc-photo-101", user_id="u1", text="chloroplast converts sunlight", score=0.9)
        ],
...
async def test_response_trace_stages_exist(
    ...
    mock_retrieve.return_value = RetrievalResult(
        ...
        confidence=0.9,
        chunks=[
            RetrievedChunk(chunk_id="c1", document_id="doc-photo-101", user_id="u1", text="photosynthesis process", score=0.9)
        ],

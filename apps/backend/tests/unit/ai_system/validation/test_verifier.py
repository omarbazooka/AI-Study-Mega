...
@pytest.mark.asyncio
async def test_verifier_empty_chunks():
    result = await verify_response("query", TaskType.CHAT, [], "output", use_llm_judge=False)
    assert result.action == VerifierAction.FALLBACK
    assert result.final_answer == FALLBACK_MESSAGE

@pytest.mark.asyncio
async def test_verifier_max_retries_exceeded(sample_chunks):
    result = await verify_response("query", TaskType.CHAT, sample_chunks, "output", retry_count=MAX_VERIFICATION_RETRIES + 1, use_llm_judge=False)
    assert result.action == VerifierAction.FALLBACK

@pytest.mark.asyncio
async def test_verifier_all_valid(sample_chunks, monkeypatch):
    ...
    result = await verify_response("query", TaskType.CHAT, sample_chunks, "This is a properly grounded and long enough output.")
